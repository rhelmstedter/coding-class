...
If a letter is not highlighted, it is not in the word at all.\n"""
...
def main():
    ...
    while True:  # Main game loop.
        ...
        secret_word = "ANOAS" #generate_secret_word()
        num_guesses = 1
...
def get_clues(guess, secret_word, bad_letters, good_letters, perfect_letters):
    """Returns a string formatted with YELLOW or GREEN."""
    if guess == secret_word:
    ...
    for i, letter in enumerate(guess):
        ...
        if guess[i] == secret_word[i]:
            clues[i] = (f"[{ GREEN }]{letter}[/{ GREEN }]")
            perfect_letters.add(letter)

    for i, letter in enumerate(guess):
        if letter in secret_word \
        and guess[i] != secret_word[i] \
        and guess_counts[letter] <= secret_counts[letter]:
            clues[i] = (f"[{ YELLOW }]{letter}[/{ YELLOW }]")
